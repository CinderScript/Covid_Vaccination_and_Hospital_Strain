...
#    simple_filter = "recip_state='WA'",                              # can add this for smaller dataset for testing (fast soda api call)
    ...
    select_filter = "fips,series_complete_pop_pct,administered_dose1_pop_pct,booster_doses_vax_pct",
    limit = 10000)
}
...
get_vaccination_rates_data <- function(date){
  ...
  filepath = paste0(cached_vaccination_data_filepath, yyyy_mm_dd, ".csv")
  ...
  if( !file.exists(filepath) ) {
    ...
    data = get_vaccination_rates_cdc_soda_api(date = yyyy_mm_dd)
...
vaccination_rates <- get_vaccination_rates_data(date = '2021-09-24')
```
