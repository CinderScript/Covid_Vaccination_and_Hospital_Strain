...
#    simple_filter = "recip_state='WA'",                              # can add this for smaller dataset for testing (fast soda api call)
    ...
    select_filter = "date,fips,series_complete_pop_pct,administered_dose1_pop_pct,booster_doses_vax_pct",
    limit = 10000)
...
test <- read.csv("cached-data/COVID-19_Vaccinations_in_the_United_States_County.csv")

by_day <- test %>% 
  filter(Date == "09/24/2021")

Query:  https://data.cdc.gov/resource/8xkx-amqh.json?&$where=date='2021-09-24'&$select=fips,series_complete_pop_pct,administered_dose1_pop_pct,booster_doses_vax_pct&$limit=10000&$$app_token=3tl81UiFuoaJ1DboeqMuy5K9U

Query:  https://data.cdc.gov/resource/8xkx-amqh.json?&$where=date='09/24/2021'&$select=date,fips,series_complete_pop_pct,administered_dose1_pop_pct,booster_doses_vax_pct&$limit=10000&$$app_token=3tl81UiFuoaJ1DboeqMuy5K9U


```{r}
create_data_folder_if_dne()

### Get data that will be studied
hospital_bed_data <- get_bed_utilization_data(date = '2021/09/24')

vaccination_data <- get_vaccination_rates_data(date = '2022/09/22')
vaccination_data_json <- get_vaccination_rates_data(date = '2021/09/24')

### Get geometry for mapping
hrr_shape_data <- get_hrr_shapes_arcgis_featureserver_api()
...
# Map Vaccination Rates

```{r}

map_data_vaccination_rate <- us_county_shape_data %>% 
  left_join(vaccination_data, by = "fips")

map_data_vaccination_rate %>% 
  ggplot() +
  geom_sf(aes(fill = series_complete_pop_pct)) +
  scale_fill_continuous(low="yellow", high="red", labels = scales::percent,
  breaks=c(20,80)) +
  my_map_theme()

map_data_vaccination_rate %>% 
  filter(iso_3166_2 == "TX") %>% 
  summarise(series_complete_pop_pct, administered_dose1_pop_pct) %>% 
