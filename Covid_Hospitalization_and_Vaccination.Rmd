...
This dataset is needed for the function that calculates vaccination rates of HRRs
...
calculate_hrr_vaccination_rates <- function(date) {
  ...
  zip_hrr_vaccination_population_join = vaccination_data %>% 
    ...
    select(ZCTA5, fips, POPPT, series_complete_pop_pct, hrrnum) %>% 
    
    rename(zcta = ZCTA5, zip_pop_slice_in_fips = POPPT, fips_vaccination_percent = series_complete_pop_pct) %>% 
  ...
  hrr_vacc_percent = zip_hrr_vaccination_population_join %>% 
    mutate(num_vacc_in_zip_slice = fips_vaccination_percent * zip_pop_slice_in_fips) %>% 
    mutate(zip_slice_weighted_vacc_percent = num_vacc_in_zip_slice / total_hrr_population) %>% 
    select(hrrnum, zip_slice_weighted_vacc_percent) %>% 
    group_by(hrrnum) %>% 
    summarise(vacc_percent = sum(zip_slice_weighted_vacc_percent))
}
...
Graph_Vaccination_Rates_By_Hrr <- function(date) {
  vaccination_data = calculate_hrr_vaccination_rates(date = date)
  ...
  hrr_ggplot_data %>% 
    ggplot() +
      geom_sf(aes(fill = vacc_percent/100), linewidth = 0, color=alpha("black",0.02)) +
      geom_sf(data = us_state_shape_data, fill = alpha("black", 0.0)) +
      scale_fill_continuous("Beds Used for Covid", low="yellow", high="red", labels = scales::percent) +
...
```{r}
Graph_Vaccination_Rates_By_Hrr("2021/03/1")

Graph_Vaccination_Rates_By_Hrr("2021/09/24")
```
