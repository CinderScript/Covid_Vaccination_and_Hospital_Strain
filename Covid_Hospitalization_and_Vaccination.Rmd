...
```{r}
test <- function(date, x_axis, y_axis){
...
test("2021-09-23", "vacc_complete_percent", "covid_bed_usage_ratio")

```

```{r}
test <- function(date, display_stat, is_scale_automatic = F) {
...
data = test("2022-09-23", "vacc_complete_percent", T)
```
  
  before 2021-10-22, TX does not have records
  by "2021-02-01", counties have some single dose percentage - TX is the only one that has 0.
...
data = test("2022-10-25", "vacc_complete_percent", T)

data %>% filter(single_dose_percent < vacc_complete_percent)
```
```{r}
