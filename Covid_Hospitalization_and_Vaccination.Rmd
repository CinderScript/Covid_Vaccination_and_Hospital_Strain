...
#cashed data
cashed_data_folder = "cashed-data"
cashed_hrr_shapes_filepath = paste(cashed_data_folder, "hhr_shapes_arcgis_data.shape", sep = "/")
cashed_hospital_bed_data_filepath = paste(cashed_data_folder, "hospital_bed_data/hospital_bed_utilization_healthdatagov", sep = "/")
...
# week on this dataset has between 3,000 and 4,000 records.
get_bed_utilization_healthdata_gov_soda_api <- function(date){
  ...
  # simple_filter = "state='WA'",  # can add for smaller dataset for testing (fast soda api call)
    where_filter = "collection_week=", date,
...
# Checks for data in the cashed data folder first, then looks for the data using the soda rest api
get_bed_utilization_data <- function(date){
  
  #convert date to string soda api likes
  yyyy_mm_dd = ymd(date)

  print(paste("Date Given: ", yyyy_mm_dd))
  
  # first we need to determine the filename of the cashed data file.
  filepath = paste0(cashed_hospital_bed_data_filepath, yyyy_mm_dd, ".csv")
  
  if( !file.exists(filepath) ) {
    
    # get from healthdata.gov
    
    print("Hospital bed cashed data was not found. Retrieving from healthdata.gov (soda api)...")
    bed_data = get_bed_utilization_healthdata_gov_soda_api(yyyy_mm_dd)
    
    write_csv(bed_data, filepath)
    print("File retrieved and cashed for future use.")
  }
  else {
    
    # get from cashed data file
    
    bed_data = read_csv(filepath)
    print("Locally cashed hospital bed data found and loaded.")
  }
...
get_hrr_shapes_arcgis_featureserver_api <- function() {
  ...
  if( !file.exists(cashed_hrr_shapes_filepath) ) {
  
    print("No cashed hrr shapefile was found. Downloading from arcgis and saving to file...")
    ...
    st_write(hrr_shape_data, cashed_hrr_shapes_filepath, driver = "GeoJSON")
  }
  else {
    hrr_shape_data = st_read(cashed_hrr_shapes_filepath)
    print("Cashed hrr shapes file was found and loaded.")
  }
...
create_data_folder_if_dne <- function(){
  if ( !dir.exists(file.path(cashed_data_folder)) ) {
    print("Creating cashed data folder...")
    dir.create(cashed_data_folder)
  }
  else
    print("Founds cashed data folder.")
}
...
Datasets are first looked for in the cashed data folder, then downloaded if not found.
