...
server <- function(input, output, session) {
  ...
  output$vacc_data_date <- renderText({
    ...
    if(active_tab() == "nav_vacc_map_tab")
      paste(
        "Date of Vaccination Records (data.cdc.gov): ", 
        closest_valid_dates(vacc_map_date())[1])
    else
      paste(
        "Date of Vaccination Records (data.cdc.gov): ", 
        closest_valid_dates(hosp_plot_date())[1])
  })
  output$bed_data_date <- renderText({
    req(active_tab())
    if(active_tab() == "nav_hosp_plot_tab")
      paste(
        "Date of Hospital Records (HealthData.gov): ", 
