...
get_hrr_shapes_arcgis <- function() {
  ...
  if( !file.exists(cached_hrr_shapes_filepath) ) {
    ...
    hrr_shape_data = st_read(request, quiet = TRUE)
    
    st_write(hrr_shape_data, cached_hrr_shapes_filepath, driver = "GeoJSON")
  }
  else {
    hrr_shape_data = st_read(cached_hrr_shapes_filepath, quiet = TRUE)
...
#static data Load Static Data       - needed for data wrangling
